...
public class Demo_4 extends JFrame implements ActionListener, ItemListener {
	...
	public Demo_4() {
		...
		cmbLanguages.addItem("SansSerif");
		cmbLanguages.addItem("Serif");
		cmbLanguages.addItem("Monospaced");
		cmbLanguages.addItem("Arial");
		cmbLanguages.addItem("Tahoma");
		cmbLanguages.addItem("Segoe UI");

		// combobox font size
		cmbFontSize = new JComboBox();
		int size = 2;
		for (int i = 0; i < 7; i++) {
			cmbFontSize.addItem(size);
			size *= 2;
		}
		...
		pBar.add(radCenter);
		pBar.add(radRight);
		pBar.add(cmbLanguages);
		pBar.add(cmbFontSize);
	...
	public void actionPerformed(ActionEvent e) {
		...
		} else if (o.equals(cmbLanguages)) {
			String fontName = (String) cmbLanguages.getSelectedItem();
			txaBoard.setFont(new Font(fontName, txaBoard.getFont().getStyle(), txaBoard.getFont().getSize()));
			txtResult.setFont(new Font(fontName, txtResult.getFont().getStyle(), txtResult.getFont().getSize()));
		} else if (o.equals(cmbFontSize)) {
			int fontSize = (int) cmbFontSize.getSelectedItem();
			Font fTArea = txaBoard.getFont();
			Font fTField = txtResult.getFont();
			txaBoard.setFont(new Font(fTArea.getFontName(), fTArea.getStyle(), fontSize));
			txtResult.setFont(new Font(fTField.getFontName(), fTField.getStyle(), fontSize));
		}
	}
